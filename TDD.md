...
**A second instance of the same pattern, found the same way.** `extract_size` mapped the word "small" to size S, which is right for "do you have this in small" and wrong for "does this run small" — that's fit language, not a size request. A message like "do you have anything that runs small" would then have been answered with size-S stock counts: a true statement, and not an answer to the question asked. Again the guardrail cannot help, because nothing stated is false. The fix is a negative context check in `extract_size` (skip the word-form mapping when the size word follows "runs"/"fits"), covered in both directions by `tests/test_fit_and_sizing.py`. Two bugs of this shape in one prototype is the useful signal: **truth-checking and relevance-checking are different problems, and this architecture only does the first.** A production version would want a lightweight relevance gate — does this reply actually address the question that was asked — as a separate check, and it's one of the few places I'd reach for a model call rather than a rule, since relevance is exactly the judgement rules are worst at.

### Fit questions: grounding an answer in prose the seller already wrote

The `fit_question` intent exists because the fit facts were already in the store — "true to size" and "runs one size small" sit in the catalog descriptions — while the copilot was handing back a generic non-answer to questions it had the data to answer ("How about petite?" was the message that exposed it). `FIT_PHRASES` (`reply/generator.py`) maps a phrase as it appears in a description to the same claim in sentence form; a fit answer is emitted only when the resolved product's description actually contains one, and `GuardrailEngine._check_fit` maps the sentence form back and re-checks it against that description — so the fit path gets the same re-derive-don't-trust treatment as price and stock. A draft claiming the coral dress runs small is blocked, because its description says it's true to size.

Two decisions worth defending. First, phrase-matching prose is the *wrong* long-term design, and PRD.md says so — the right one is a structured fit attribute per SKU and per size. It was the correct call for this prototype because it grounds the answer in data the seller already maintains with no new data-entry burden, and whether a pilot seller keeps using the thing turns on exactly that kind of friction. Second, `classify_intent` checks fit *before* availability, so "do you have this in petite" resolves to a sizing answer ("we don't carry petite sizing — the Coral Wrap Midi Dress comes in XS, S, M, L, XL") rather than a stock count for a size the catalog doesn't contain. That ordering is a judgement call with a real cost: a message mixing both ("does it run small, and do you have an M?") answers only the fit half.
